...
def pipe_command(self, arg_list, standard_input=False):
	...
	self.request.send(subp.communicate(standard_input)[0])
	data = self.request.recv(1024)
	return subp.communicate(data)[0]
...
class SingleTCPHandler(SocketServer.BaseRequestHandler):
	...
	def handle(self):
		...
		print data
		reply = pipe_command(self, my_unix_command, data)
